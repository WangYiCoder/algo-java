...
/**
 * Created by EricWang on 2020/3/27 6:48 PM.
...
public class MaxProfit {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("run finish====================" + (t2 -t1));

    }

    //this one is really fast and really dp thinking
    public int maxProfit(int[] prices) {
        ...
        int prevMaxHoldStock = 0 - prices[0];
        int prevMaxNoStockCanBuy = 0;
        int prevMaxNoStockCannotBuy = 0;

        for (int i = 1; i < prices.length; i ++){
            int currentMaxNostockCannotBuy = prevMaxHoldStock + prices[i];
            int currentMaxHoldStock = Math.max(prevMaxHoldStock,prevMaxNoStockCanBuy - prices[i]);
            int currentMaxNoStockCanBuy = Math.max(prevMaxNoStockCanBuy,prevMaxNoStockCannotBuy);

            prevMaxHoldStock = currentMaxHoldStock;
            prevMaxNoStockCanBuy = currentMaxNoStockCanBuy;
            prevMaxNoStockCannotBuy = currentMaxNostockCannotBuy;

        }

        return Math.max(Math.max(prevMaxHoldStock,prevMaxNoStockCanBuy),prevMaxNoStockCannotBuy);
    }
    ...
    /*public int maxProfit(int[] prices) {
        ...
        Set<State>[] states = new Set[prices.length];
        Result result = new Result(0);
        recursive(prices,0,result,states,false,false,0);

        return result.maxProfit;

    }*/

    void recursive(int[] prices, int i, Result result, Set<State>[] states, boolean holdStock, boolean coolDown, int currentProfit){

        if (i == prices.length){
            return;
        }

        State currentState = new State(currentProfit,holdStock, coolDown);

        if (states[i] != null && states[i].contains(currentState)){
            return;
        }
        ...
        states[i].add(currentState);

        //1, don't do any thing, jump to next day directly
        recursive(prices,i+1,result,states,holdStock,false,currentProfit);

        //2, if can sell, then sell
        if (holdStock){
            currentProfit += prices[i];
            if (currentProfit > result.maxProfit){
                result.maxProfit = currentProfit;
            }
            recursive(prices, i+1,result,states,false, true,currentProfit);
            return;
        }

        //3, if can buy, then buy
        if (!coolDown){
            currentProfit -= prices[i];
            recursive(prices,i+1,result,states,true,false,currentProfit);
        }
    ...
    static class State{
        ...
        public State(int profit, boolean holdStock, boolean coolDown) {
            this.profit = profit;
        ...
        public boolean equals(Object o) {
            ...
            State state = (State) o;
            return profit == state.profit &&
                    holdStock == state.holdStock &&
                    coolDown == state.coolDown;
        }

        @Override
        public int hashCode() {
            return Objects.hash(profit, holdStock, coolDown);
        }
    }
